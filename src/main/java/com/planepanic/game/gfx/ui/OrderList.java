...
public class OrderList extends Drawable {
	...
	public void draw2d() {
		Plane plane;
		int y = (int) (getPosition().getY() - getHitboxSize().getY() / 2) + offset;
	...
	private void drawOrder(int y, Order order) {
		DrawUtil.setColor(0xBBBBBB);
		DrawUtil.drawSquare((float) this.getPosition().getX(), y, (float) this.getHitboxSize().getX(), 100, true, this.getPriority());
	...
	protected boolean onScroll(int scroll) {
		offset = Math.min(Math.max(Math.min((int) (this.getHitboxSize().getY() - Plane.getSelected().getOrders().size() * 100 + 50), 50), offset + scroll / 12), 50);
		return true;
	}
